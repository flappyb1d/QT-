...
//提示
    ...
    connect(ui->send_button,&QPushButton::clicked,this,&ChatServerWidget::sendMessage);
    connect(ui->clear_button,&QPushButton::clicked,this,&ChatServerWidget::clear_Msg);
...
//窗口重绘
void ChatServerWidget::resizeEvent(QResizeEvent *event)
{
    Q_UNUSED(event);
...
//开启服务器
void ChatServerWidget::startServer()
{
    //已打开服务器
...
//    || listenFlag_file == false
    ...
    connect(server_check,&QTcpServer::newConnection,
            this,&ChatServerWidget::newClient_check);
    ...
    connect(server_check,&QTcpServer::destroyed,
            this,&ChatServerWidget::destoryServer_check);

}
//通过数据库验证账号部分

//新客户端连接
void ChatServerWidget::newClient_check()
{
    ...
    //有可读信息，发射readyread信号
    connect(newClient,&QTcpSocket::readyRead,this,&ChatServerWidget::readMessage_check);
    //断开连接时，发射disconnect信号
    connect(newClient,&QTcpSocket::disconnected,this,&ChatServerWidget::lostClient_check);
...
//账号验证的反馈消息
void ChatServerWidget::feedbackMessage(QString msg)
{
    //服务端未打开
    if(!listenFlag_check) {
        return;
...
//发送
void ChatServerWidget::sendMessage_check()
{
    //服务端未打开
    if(!listenFlag_check) {
        return;
...
//读取
void ChatServerWidget::readMessage_check()
{
...
//删除
void ChatServerWidget::lostClient_check()
{
    //登录销毁
    QTcpSocket *client = qobject_cast<QTcpSocket *>(sender());
...
//新客户端连接
void ChatServerWidget::newConnect_news()
{
...
//发送
void ChatServerWidget::sendMessage_news(QString information)
{
    //服务端未打开
    if(!listenFlag_news) {
        return;
...
//读取
void ChatServerWidget::readMessage_news()
{
    QString message;
    //遍历客户端列表，所有客户端
    for(int i = 0;i < clients_news->length();i++)
...
//删除
void ChatServerWidget::lostClient_news()
{
    //关闭聊天
    QTcpSocket *client = qobject_cast<QTcpSocket *>(sender());
...
//新客户端连接
void ChatServerWidget::newConnect_file()
{
...
//发送
void ChatServerWidget::sendMessage_file(QString information)
{
    //服务端未打开
    if(!listenFlag_file) {
        return;
...
//读取
void ChatServerWidget::readMessage_file()
{
    QString message;
    //遍历客户端列表，所有客户端
    for(int i = 0;i < clients_file->length();i++)
...
//删除
void ChatServerWidget::lostClient_file()
{
    //关闭聊天
    QTcpSocket *client = qobject_cast<QTcpSocket *>(sender());
...
//新客户端连接
void ChatServerWidget::newConnect_surface()
{
    ...
    connect(client_surface,&QTcpSocket::disconnected,
            this,&ChatServerWidget::lostClient_surface);
}

//发送
void ChatServerWidget::sendMessage_surface(QString Msg)
{
...
//读取
void ChatServerWidget::readMessage_surface()
{
...
//删除
void ChatServerWidget::lostClient_surface()
{
    QTcpSocket *client = qobject_cast<QTcpSocket *>(sender());
...
//服务器端，群发、清屏
//群发
void ChatServerWidget::sendMessage()
{
    QString msg = "server";
...
//清空
void ChatServerWidget::clear_Msg()
{
    ui->display_screen->clear();
}

//释放服务器
void ChatServerWidget::destoryServer_check()
{
    //监听标志转false
