...
#include "ui_chatloading.h"
#include <QWidget>

ChatLoading::ChatLoading(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::ChatLoading)
{
    ui->setupUi(this);
...
//        label->move(88,233);
    ...
    //定时器
        ...
        connect(timer,&QTimer::timeout,
                this,&ChatLoading::progressUpdate);
}

//绘制窗口
void ChatLoading::paintEvent(QPaintEvent *)
{
...
//鼠标按键
void ChatLoading::mousePressEvent(QMouseEvent *event)
{
    //鼠标左键按下
...
//鼠标移动
void ChatLoading::mouseMoveEvent(QMouseEvent *event)
{
    //鼠标移动时会检测所有按下的键值
...
//进度条加载
void ChatLoading::progressUpdate()
{
    //加载进度到150
    if(progress++ == 1500){
        //关闭加载页面
